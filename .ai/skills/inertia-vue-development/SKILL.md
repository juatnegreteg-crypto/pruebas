...
## Async Export UX Pattern (Vue + shadcn-vue)

Use this for XLSX/PDF/CSV exports triggered from Vue pages.

1. Feedback:
   - Use shadcn-vue toast for success and error outcomes.
   - Do not show async action failures as inline error arrays.
2. Download safety:
   - Validate `Content-Type` before creating a download blob.
   - If the response is JSON/problem payload, parse and show toast error instead of downloading.
3. Browser cleanup:
   - Revoke object URLs after download (`URL.revokeObjectURL`).
4. Loading states:
   - Keep a single, explicit `isExporting` state per action.
   - Disable the trigger while in-flight and restore state in `finally`.
5. Reuse:
   - If multiple resources support export, extract behavior to a shared composable (for example `useResourceExport`).
